...
const CollectionItem = ({ id, name, img, machines, machinesNumber, isSelection, setMachines, selectedMachines }) => {
  ...
  return (
    isSelection ? (
      ...
      >
        <img src={img} alt={name} loading='lazy' className="w-40 object-cover rounded-box" />
        ...
        {isSelected && (<img src="/images/check.png" loading='lazy' className='w-10 h-10 absolute right-2 top-2' />)}
        <img src="/images/circle.png" loading='lazy' className={`w-10 h-10 absolute right-2 top-2 ${isSelected ? 'hidden' : ''}`} />
      </div>
    ) :  
      (
